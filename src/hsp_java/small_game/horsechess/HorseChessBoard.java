package hsp_java.small_game.horsechess;
...
public class HorseChessBoard {
    ...
    public static void main(String[] args) {
        ...
        for (int[] rows : chessBoard) {
            for (int step : rows) { //step 表示该位置是马儿应该走的第几步
    ...
    //编写一个方法，对ps的各个位置，可以走的下一个位置的次数进行排序，把可能走的下一个位置可以走的位置个数从小到大排序。
    //贪心算法
    ...
    //编写最核心的方法，遍历棋盘，如果遍历成功，就把 finished 设置为 true
    //并且，将马儿走的每一步step，记录到 chessBoard中
    ...
    //编写方法，可以获取当前位置，可以走的下一步的所有的位置(Point表示 x,y)
    public static ArrayList<Point> next(Point curPoint) {

        //创建一个ArrayList来存放下一步的所有位置
        ArrayList<Point> ps = new ArrayList<>();

        //创建一个Point对象(点/位置)，准备放入到 ps
        Point p1 = new Point();

        //判断在 curPoint 是否可以走如下的位置，如果可以走，就将该点(Point)放入到ps
        //判断是否可以走0位置
