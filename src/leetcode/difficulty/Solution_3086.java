 ...
 * 游戏开始时，Alice 可以选择数组 [0, n - 1] 范围内的任何索引 aliceIndex 站立。如果 nums[aliceIndex] == 1 ，Alice 会拾起一个 1 ，
 * 并且 nums[aliceIndex] 变成0（这 不算 作一次行动）。
 ...
 * 选择 x == 1 和 y == 0 ，并执行第二种类型的动作。nums 变为 [1,0,0,0] 。
 * 由于 y == aliceIndex，Alice 拾起了一个 1 ，nums 变为 [0,0,0,0] 。
 * 再次选择 j == 1 并执行第一种类型的动作。nums 变为 [0,1,0,0] 。
 * 再次选择 x == 1 和 y == 0 ，并执行第二种类型的动作。nums 变为 [1,0,0,0] 。
 * 由于y == aliceIndex，Alice 拾起了一个 1 ，nums 变为 [0,0,0,0] 。
 */
public class Solution_3086 {
