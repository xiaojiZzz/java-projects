...
public class Solution_2528 {
    public long maxPower(int[] stations, int r, int k) {
        ...
        for (int i = 0; i < n; i++) {
            nums[i] = preSum[Math.min(i + r + 1, n)] - preSum[Math.max(0, i - r)];
            min = Math.min(min, nums[i]);
        }
        long left = min, right = min + k + 1;
        while (left + 1 < right) {
            long mid = left + (right - left) / 2;
            if (check(mid, nums, n, r, k)) {
                left = mid;
            } else {
                right = mid;
            }
    ...
    public boolean check(long min, long[] nums, int n, int r, int k) {
        long[] diff = new long[n + 1];
        long sumD = 0, need = 0;
        for (int i = 0; i < n; i++) {
            sumD += diff[i];
            long m = min - nums[i] - sumD;
            if (m > 0) {
                sumD += m;
                need += m;
                if (need > k) {
