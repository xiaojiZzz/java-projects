...
public class Solution_502 {
    public int findMaximizedCapital(int k, int w, int[] profits, int[] capital) {
        ...
        PriorityQueue<int[]> priorityQueue1 = new PriorityQueue<>((a, b) -> a[0] - b[0]);
        PriorityQueue<Integer> priorityQueue2 = new PriorityQueue<>((a, b) -> b - a);
        for (int i = 0; i < n; i++) {
            priorityQueue1.add(new int[]{capital[i], profits[i]});
        }
        for (int i = 0; i < k; i++) {
            while (!priorityQueue1.isEmpty() && priorityQueue1.peek()[0] <= w) {
                priorityQueue2.add(priorityQueue1.poll()[1]);
            }
            if (!priorityQueue2.isEmpty()) {
                w += priorityQueue2.poll();
            } else {
                break;
            }
        }
