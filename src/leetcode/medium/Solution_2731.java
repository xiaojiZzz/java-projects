 ...
 * 给你一个字符串 s ，每个字符按顺序分别表示每个机器人移动的方向。'L' 表示机器人往左或者数轴的负方向移动，
 * 'R' 表示机器人往右或者数轴的正方向移动。
 ...
 * 例如，如果一个机器人位于位置 0 并往右移动，另一个机器人位于位置 1 并往左移动，下一秒，第一个机器人位于位置 0 并往左行驶，
 * 而另一个机器人位于位置 1 并往右移动。
 * 示例 1：
...
public class Solution_2731 {
    public int sumDistance(int[] nums, String s, int d) {
        ...
        for (int i = 0; i < len; i++) {
            post[i] = nums[i] + (s.charAt(i) == 'R' ? d : -d);
        }
        Arrays.sort(post);
        long res = 0;
        long tmp_dis = 0;
