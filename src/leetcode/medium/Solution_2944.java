...
class Solution {
    public int minimumCoins(int[] prices) {
        ...
        queue.offerLast(new int[]{n + 1, 0});
        for (int i = n; i > 0; i--) {
            while (queue.peekLast()[0] > 2 * i + 1) {
                queue.pollLast();
            }
            int price = prices[i - 1] + queue.peekLast()[1];
            while (price <= queue.peekFirst()[1]) {
                queue.pollFirst();
            }
            queue.offerFirst(new int[]{i, price});
        }
