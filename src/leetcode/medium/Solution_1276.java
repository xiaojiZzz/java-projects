...
public class Solution_1276 {
    public List<Integer> numOfBurgers(int tomatoSlices, int cheeseSlices) {
        ...
        while (left < right) {
            ...
            int leftTomato = tomatoSlices - 4 * mid;
            int leftCheese = cheeseSlices - mid;
            if (leftTomato >= 0) {
                if (leftTomato % 2 == 0) {
                    if (leftTomato / 2 > leftCheese) {
                        left = mid + 1;
                    } else {
                        right = mid;
                    }
        ...
        if ((cheeseSlices - left) * 2 + left * 4 == tomatoSlices) {
            bigNum = left;
            smallNum = cheeseSlices - left;
        } else {
            return ans;
        }
        ans.add(bigNum);
