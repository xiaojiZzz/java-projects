    ...
    public long totalCost(int[] costs, int k, int candidates) {
        ...
        if (2 * candidates < len) {
            for (int i = 0; i < candidates; i++) {
                priorityQueue1.add(costs[i]);
                priorityQueue2.add(costs[len - i - 1]);
            }
            int s = candidates, t = len - candidates - 1;
            while (s <= t && k > 0) {
                if (priorityQueue1.peek() <= priorityQueue2.peek()) {
                    ans += priorityQueue1.poll();
                    priorityQueue1.add(costs[s++]);
                } else {
                    ans += priorityQueue2.poll();
                    priorityQueue2.add(costs[t--]);
                }
                k--;
            }
            if (k == 0) {
